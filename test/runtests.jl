...
function CommaToAnd(string::String)
    println("CommaChanging:'$string'")
    return join([CommaToAnd(char) for char in string])
end

function CommaToAnd(c::Char)
    c == ',' && return " and " ###the comma is an and
    c == ';' && return " and " ###the comma is an and
    return c
end


function RemoveBlanks(stringIn::AbstractString)
